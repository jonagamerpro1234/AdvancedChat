package jss.advancedchat.libs.iridium;

import com.google.common.collect.ImmutableMap;
import jss.advancedchat.libs.iridium.patterns.*;
...
public class IridiumColorAPI {
    ...
    public static @NotNull String color(@NotNull String string, @NotNull Color color) {
        return (SUPPORTS_RGB ? ChatColor.of(color) : getClosestColor(color)) + string;
    }

    public static @NotNull String color(@NotNull String string, @NotNull Color start, @NotNull Color end) {
        ChatColor[] colors = createGradient(start, end, withoutSpecialChar(string).length());
        return apply(string, colors);
    }

    public static @NotNull String rainbow(@NotNull String string, float saturation) {
    ...
    @Contract(pure = true)
    public static @NotNull String stripColorFormatting(@NotNull String string) {
        return string.replaceAll("<#[\\dA-F]{6}>|[&§][a-f\\dlnokm]|</?[A-Z]{5,8}(:[\\dA-F]{6})?\\d*>", "");
    }

    private static @NotNull String apply(@NotNull String source, ChatColor[] colors) {
    ...
    private static ChatColor @NotNull [] createGradient(@NotNull Color start, @NotNull Color end, int step) {
        ChatColor[] colors = new ChatColor[step];
