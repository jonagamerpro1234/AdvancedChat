...
public class ChatListenerTest implements Listener {
    ...
    public void onChat(@NotNull AsyncPlayerChatEvent e) {
        ...
        Logger.debug(msg);
        String message = " &r" + msg;
        Logger.debug(message);

        format = Util.getVar(j, format);
        message = Util.getVar(j, message);

        boolean isMute = playerManager.loadPlayerData(j).getPreferences().getMuteSettings().isMute();
       ...
       /* if (!playerManager.loadPlayerData(j.getName())) {
    ...
    public void chatMention(@NotNull AsyncPlayerChatEvent e) {
        ...
        /*if (message.contains(j.getName()) && !playerManager.loadPlayerData(j.getName()).isMentionEnabled()) {
    ...
    private void handleChatEvent(@NotNull FileConfiguration config, Json json, Player player) {
        ...
        cmd_action = Util.getVar(player, cmd_action);
        suggest_action = Util.getVar(player, suggest_action);

        if (hover) {
            if (click) {
                assert click_mode != null;
                handleClickAction(json, click_mode, cmd_action, url_action, suggest_action);
            } else {
    ...
    private void handleClickAction(Json json, @NotNull String click_mode, String cmd_action, String url_action, String suggest_action) {
